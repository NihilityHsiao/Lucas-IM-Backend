...
impl EtcdServiceRegister {
    ...
    pub async fn from_config(config: &EtcdConfig) -> anyhow::Result<Self> {
        ...
        let client = etcd_client::Client::connect(&config.hosts,Some(options)).await.map_err(|e| anyhow!("connect to etcd failed: {}",e.to_string()))?;
        let op = Options{
            namespace: "/lucasim/microservices".to_string(),
            ttl: DEFAULT_TTL_SECOND,
            max_retry: 5,
        ...
        tokio::spawn(async move {
            loop {
                let (mut lease_keeper,mut lease_keep_alive_stream) = loop {
                    ...
                    if keeper.is_err() {
                        warn!("etcd client lease keep alive error: {}",keeper.unwrap_err());
                ...
                let lease_result = Self::keep_alive_loop(lease_id,keep_alive_interval,lease_keeper,lease_keep_alive_stream).await;
                match lease_result {
                    Ok(_) => {
    ...
    async fn keep_alive_loop(lease_id: LeaseId, interval_second: u64, mut lease_keeper: LeaseKeeper, mut lease_keep_alive_stream: LeaseKeepAliveStream) -> anyhow::Result<()>{
        loop {
                ...
                Ok(resp) => {
                    match resp {
                        ...
                        Some(resp) => {
                            ...
                            //  只有这种情况是续约成功
                            debug!("leaseId {:?} keep alive success, new ttl: {}",lease_id,resp.ttl());
                        }
                    }
                }
                Err(e) => {
...
impl ServiceRegister for EtcdServiceRegister {
    async fn register(&mut self, service: ServiceInstance) -> anyhow::Result<()> {
        ...
        let lease_id = self.lease_grant_and_keepalive(self.options.ttl, DEFAULT_KEEPALIVE_INTERVAL_SECOND).await.map_err(|e| anyhow!("etcd lease grant failed: {}",e.to_string()))?;
