...
mod tests {
    ...
    async fn test_discover_user_rpc() -> anyhow::Result<()> {
        ...
        let mut config = Config::new(r"D:\SVN\Code\rust\Lucas-IM-Backend\apps\user\rpc\etc\user.yml");

        let etcd_client = etcd_client::Client::connect(config.etcd.hosts.clone(),None).await.expect("Failed to connect to etcd.hosts.host");
        ...
        let user_channel = discovery.get_service_channel("user.rpc").await.expect("success");

        let mut user_rpc = UserServiceClient::new(user_channel);

        let x = user_rpc.find_user(FindUserRequest {
